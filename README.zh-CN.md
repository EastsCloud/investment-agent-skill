...
[English (primary)](README.md) · 简体中文

快速入口：[自己测试](docs/TESTING.zh-CN.md) · [Prompt 在哪里](docs/PROMPTS.zh-CN.md) · [验证记录](VALIDATION.zh-CN.md)
...
核心 prompt 是 [SKILL.md](plugins/investment-research-workbench/skills/investment-research-workbench/SKILL.md)，
Codex 选择 Skill 后读取它；references 提供详细流程，初始化生成的 AGENTS.md 保存项目长期规则。
不必每次粘贴一个大型 prompt。scripts 是执行工具，tests 是代码测试，不能替代这些指令。
哪个文件负责什么、应该改哪里，见 [Prompt 导航](docs/PROMPTS.zh-CN.md)。
下面发送到 **Codex 聊天框**，不要发到终端。
...
- [.github/workflows/test.yml](.github/workflows/test.yml)：GitHub Actions 配置，在 push 和 pull request 后自动执行 Ubuntu/Windows 校验与测试；普通使用不必手动运行它。
- [.agents/plugins/marketplace.json](.agents/plugins/marketplace.json)：插件来源目录，用于安装时定位包。
- [plugins/investment-research-workbench/](plugins/investment-research-workbench/)：用户真正安装的插件包，包含 manifest、Skill 指令、参考资料、脚本和模板。
- [SKILL.md](plugins/investment-research-workbench/skills/investment-research-workbench/SKILL.md)：主 prompt；[references/](plugins/investment-research-workbench/skills/investment-research-workbench/references/)：按需读取的详细指令。
- assets/project_scaffold/：初始化时复制进私人项目的 AGENTS、模板和 RAG 工具。
- tests/ 与 tools/：开发者自动化测试、分发校验和发布扫描。
- [docs/](docs/)：中英文 prompt 和测试指南。

plugin manifest 的默认 prompt 只是聊天起始示例；SKILL.md 和私人项目 AGENTS.md 才是主要工作指令。
...
普通用户先按 [手动试用指南](docs/TESTING.zh-CN.md) 检查真实 prompt 行为。
下面的代码测试命令在 **插件仓库根目录**运行；RAG 命令则在初始化后的私人项目运行。
...
英文为主，同一次修改里同步中文。README、prompt 导航、测试指南、验证记录提供配对文件；
bundled references 和项目模板采用英文在前、中文在后。主 SKILL.md 保持唯一的精简英文入口，
链接中文说明，不另建第二个翻译 Skill。维护规则见 [CONTRIBUTING.md](CONTRIBUTING.md)。
